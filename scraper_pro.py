...
for index in range(1,len(temp_list)):
    star_name.append(temp_list[index][1])
    star_distance.append(temp_list[index][3])
    star_mass.append(temp_list[index][5])
    star_radius.append(temp_list[index][6])

# print(filtered_star_data)
df=pd.DataFrame(list(zip(star_name,star_distance,star_mass,star_radius)), columns=['Star_name','Distance','Mass','Radius'])
